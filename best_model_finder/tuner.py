...
class Model_Finder:
    ...
    def get_best_params_for_linear_regression(self,train_x,train_y,test_x,test_y):
        self.logger_object.log(self.file_object, 'Entered the get_best_params_for_linear_regression method of the Model_Finder class')
        try:
            ...
            self.grid = GridSearchCV(estimator=self.lr, param_grid=self.param_grid, cv=2,  n_jobs = 5, verbose=True)
            ...
            self.lr = LinearRegression(n_estimators=self.n_estimators, criterion=self.criterion,
                                              max_depth=self.max_depth, max_features=self.max_features)
            # training the mew model
            self.lr.fit(train_x, train_y)
            self.lr_score = self.lr.score(test_x, test_y)
            self.logger_object.log(self.file_object,
                                   'Linear Regression best params: '+str(self.grid.best_params_)+'. Exited the get_best_params_for_linear_regression method of the Model_Finder class')

            return self.lr, self.lr_score
        except Exception as e:
            self.logger_object.log(self.file_object,
                                   'Exception occured in get_best_params_for_linear_regression method of the Model_Finder class. Exception message:  ' + str(
                                       e))
            self.logger_object.log(self.file_object,
                                   'Random Forest Parameter tuning  failed. Exited the get_best_params_for_linear_regression method of the Model_Finder class')
            raise Exception()
